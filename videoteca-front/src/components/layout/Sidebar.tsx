...
const NAV_ITEMS: Record<string, NavItem[]> = {
  ...
  funcionario: [
    ...
    { label: "Solicitações (Prof.)", href: "/funcionario/solicitacoes-professores", icon: Inbox },
...
export function Sidebar({ role, onNavigate }: SidebarProps) {
  ...
  return (
    <aside className="w-64 min-h-screen h-full bg-[#003A70] flex flex-col text-white flex-shrink-0 shadow-xl lg:shadow-none">
      {/* Logo */}
      ...
      <nav className="flex-1 px-3 pb-4 overflow-y-auto" aria-label="Menu principal">
        <ul className="space-y-0.5">
          {items.map((item) => {
            const dashboardPaths = ["/aluno", "/professor", "/funcionario"];
            const isActive = dashboardPaths.includes(item.href)
              ? pathname === item.href
